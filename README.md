...
Gic Wallet Pro is a browser extension that enables secure interaction with Gic-enabled web services.
...
Gic Wallet Pro is designed for convenience, so users can sign transactions with just one click. Users can create multiple wallets and switch between them easily. And if a user ever forgets the password to the account, it can be recovered from the seed phrase.

[Gic protocol documentation](https://docs.gicsports.io/en/)

## Gic Wallet Pro API

On browser pages that operate under `http/https` (not local pages with `file://` protocol) with Gic Wallet Pro extension installed, `GicWalletPro` global object becomes available.

> `GicWalletPro` global object is **deprecated** and is not recommended for future use.

In `GicWalletPro` you will find the following methods:
...
On initialize `window.GicWalletPro` has no API methods.
You can use `GicWalletPro.initialPromise` to wait for API initialization to complete.

Example:

```js
GicWalletPro.initialPromise.then(keeperApi => {
  /*...init app*/
  keeperApi.publicState().then(state => startApp(state));
});
```

In Gic Wallet Pro, for greater security and ease of use, each new website using API has to be allowed by the user. At the first attempt to use API (except `on`), the user will see a request to allow the website to access Gic Wallet Pro. If the user agrees to allow access, the website is considered trusted and can use API on its pages. Otherwise, the website is blocked, and the error message `{message: "Api rejected by user", code: 12}` will be displayed in response to all requests. The user won't see new notifications. To grant access, the user has to mark the website as trusted in the interface.

### Description of methods

#### publicState

If a website is trusted, Gic Wallet Pro public data are returned.

Example:

```js
GicWalletPro.publicState()
  .then(state => {
...
const getPublicState = async () => {
  try {
    const state = await GicWalletPro.publicState();
    console.log(state); // displaying the result in the console
    /*... processing data*/
  } catch (error) {
    console.error(error); // displaying the result in the console
...
- `initialized` – boolean: Gic Wallet Pro is initialized.
- `locked` – boolean: Gic Wallet Pro is locked (password required).
- `account` – current account if the user allowed access to the website, `null` otherwise.
- `network` – current Gic network, node and matcher addresses.
- `messages` – signature request statuses.
- `txVersion` – available transaction versions for each type.

Possible errors:

- `{ message: "Init Gic Wallet Pro and add account" }` – Gic Wallet Pro is not initialized.
- `{ message: "Add Gic Wallet Pro account" }` – Gic Wallet Pro accessed, but there are no accounts.
- `{ message: "User denied message" }` – the user denied the website operation with Gic Wallet Pro.
...
You can encrypt string messages to account in Gic network.
You need have recipient publicKey.

GicWalletPro.encryptMessage(`*string to encrypt*`, `*public key in base58 string*`, `*prefix: a secret app string for encoding*`)

Example:

```js
GicWalletPro.encryptMessage(
  'My message',
...
- `{ message: "Init Gic Wallet Pro and add account" }` – Gic Wallet Pro is not initialized.
- `{ message: "App is locked" }` – Gic Wallet Pro is locked (password required).
- `{ message: "Add Gic Wallet Pro account" }` – Gic Wallet Pro accessed, but there are no accounts.
- `{ message: "User denied message" }` – the user denied the website operation with Gic Wallet Pro.

#### decryptMessage

You can decrypt string messages from account in Gic network to you.
You need to have user's public key and the encrypted message.

GicWalletPro.decryptMessage(`*string to decrypt*`, `*public key in base58 string*`, `*prefix: a secret app string for encoding*`)

Example:

```js
GicWalletPro.decryptMessage(
  '**encrypted msg**',
  '416z9d8DQDy5MPTqDhvReRBaPb19gEyVRWvHcewpP6Nc',
  'my app'
).then(message => {
...
Allows subscribing to Gic Wallet Pro events.
...
```js
GicWalletPro.on('update', state => {
  //state object as from GicWalletPro.publicState
});
```
...
This is a method for obtaining a signature of authorization data while verifying Gic' user. If the the signature is valid, be sure that the given blockchain account belongs to that user.

Example:

```js
const authData = { data: 'Auth on my site' };
GicWalletPro.auth(authData)
  .then(auth => {
...
const getAuthData = async authData => {
  try {
    const state = await GicWalletPro.auth(authData);
...
const authData = {
  ...
  name: 'My test App',
  icon: '/img/icons/Gic_logo.svg',
  referrer: 'https://futfinance.com/',
  successPath: 'login',
};

GicWalletPro.auth(authData)
  .then(data => {
    //data – data from Gic Wallet Pro
...
If the verification is successful, Gic Wallet Pro returns to the `Promise` an object containing data for signature verification:
...
- `address` – address in Gic network.
...
const data = await GicWalletPro.auth({ data: '123' });
...
def verifyAddress(public_key, address):
    public_key_bytes = base58.b58decode(public_key)
    unhashed_address = chr(1) + str('G') + crypto.hashChain(public_key_bytes)[0:20]
...
A method for signing transactions in Gic network. See the description of supported transaction types in [Transactions](#transactions) section below.
...
GicWalletPro.signTransaction(txData)
  .then(data => {
    //data – a line ready for sending to Gic network's node (server)
  })
  .catch(error => {
    //Processing errors
  });
...
In the example, we are signing a transaction for transferring Gic to the alias `test` in Gic network.
...
GicWalletPro.signAndPublishTransaction(txData)
  .then(data => {
    //data - a line ready for sending to Gic network's node (server)
  })
  .catch(error => {
...
Response: a reply from Gic network returns as a line containing the entire past transaction.
...
GicWalletPro.signTransactionPackage(tx, name);
```

Sign two transaction:

- Transfer 1.567 Gic to the alias `test`.
- Transfer 0.1 Gic to the alias `merry`.
...
Every user of Gic network has a state (balances, assets, data, scripts), and every past transaction changes this data. [More about transactions](https://docs.gicsports.io/en/blockchain/transaction/)

In Gic Wallet Pro API transaction format is different from [Node REST API](https://docs.gicsports.io/en/gic-node/node-api/). `signTransaction`, `signAndPublishTransaction`, and `signTransactionPackage` accept transactions as follows:
...
Gic Wallet Pro supports the following types of transactions:
...
- \* - optional field, data is automatically supplied from `GicWalletPro`.
...
In both messages, the same price of 1 Gic is indicated. You can easily convert `coins` into `tokens` and back, if you know in what asset the price is indicated, and you have received its precision: `tokens = coins / (10 ** precision)`.

If the field contains other types than MoneyLike, for instance, string/MoneyLike, the sum is indicated as a number in  `coins`.

##### Transaction fee

See [Transaction fee](https://docs.gicsports.io/en/blockchain/transaction/transaction-fee) section in the Gic protocol documentation.

##### Issue transaction (type 3)

See [Issue transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/issue-transaction) in the Gic protocol documentation.
...
- `*fee`: MoneyLike – fee.
- `*script`: string – asset script, see the [Smart Asset](https://docs.gicsports.io/en/building-apps/smart-contracts/what-is-smart-asset) article.
...
```js
GicWalletPro.signAndPublishTransaction({
  type: 3,
  data: {
    name: 'Best Token',
...
See [Transfer transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/transfer-transaction) in the Gic protocol documentation.
...
GicWalletPro.signAndPublishTransaction({
  ...
  .then(tx => {
    console.log("Hurray! I've been able to send Gic!!!");
  })
  .catch(error => {
    console.error('Something went wrong', error);
  });
...
See [Reissue transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/reissue-transaction) in the Gic protocol documentation.
...
See [Burn transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/burn-transaction) in the Gic protocol documentation.
...
See [Lease transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/lease-transaction) in the Gic protocol documentation.
...
In case of success, 0.00001000 Gic is leased.

##### Lease Cancel transaction (type 9)

See [Lease Cancel transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/lease-cancel-transaction) in the Gic protocol documentation.
...
See [Create Alias transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/create-alias-transaction) in the Gic protocol documentation.

Fields:

- `alias`: [4, 30 bytes] string – alias. See [alias requirements](https://docs.gicsports.io/en/blockchain/account/alias#alias-requirements).
- `*fee`: MoneyLike – fee.
- `*senderPublicKey`: string - user's public key in base58.
- `*timestamp`: number/string – time in ms.
...
See [Mass Transfer transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/mass-transfer-transaction) in the Gic protocol documentation.
...
In case of success, 0.00200000 Gic will be sent to alias1 and alias2.

##### Data transaction (type 12)

See [Data transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/data-transaction) in the Gic protocol documentation.
...
See [Set Script transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/set-script-transaction) in the Gic protocol documentation.

Fields:

- `script`: string – account script or dApp script (see the [Smart Account](https://docs.gicsports.io/en/building-apps/smart-contracts/what-is-smart-account) and [dApp](https://docs.gicsports.io/en/building-apps/smart-contracts/what-is-a-dapp) articles in the Gic protocol documentation.
- `*fee`: MoneyLike – fee.
- `*senderPublicKey`: string – user's public key in base58.
- `*timestamp`: number/string – time in ms.

To develop and compile the script, use [Gic IDE](https://gicide.com/).
...
See [Sponsor Fee transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/sponsor-fee-transaction) in the Gic protocol documentation.

Fields:

- `minSponsoredAssetFee`: MoneyLike – amount of the sponsored asset that is equivalent to 0.001 Gic.
- `*fee`: MoneyLike – fee.
- `*senderPublicKey`: string – user's public key in base58.
- `*timestamp`: number/string – time in ms.
...
See [Set Asset Script transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/set-asset-script-transaction) in the Gic protocol documentation.

Fields:

- `assetId`: string – asset ID in base58.
- `script`: string – asset script, see the [Smart Asset](https://docs.gicsports.io/en/building-apps/smart-contracts/what-is-smart-asset) article in the Gic protocol documentation.
- `*fee`: MoneyLike – fee.
- `*senderPublicKey`: string – user's public key in base58.
- `*timestamp` number/string – time in ms.

It's impossible to remove the asset script; you can only change the script. To develop and compile the script, use [Gic IDE](https://gicide.com/).
...
See [Invoke Script transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/invoke-script-transaction) in the Gic protocol documentation.
...
See [Update Asset Info transaction details](https://docs.gicsports.io/en/blockchain/transaction-type/update-asset-info-transaction) in the Gic protocol documentation.
...
Gic Wallet Pro's method for signing an order to the matcher (exchange service). As input, it accepts an object similar to a transaction like this:
...
See [order details](https://docs.gicsports.io/en/blockchain/order) in the Gic protocol documentation.

See [how to calculate the order fee](https://docs.futfinance.com/en/gic-matcher/matcher-fee) in Futfinance.com documentation.
...
Gic Wallet Pro's method for creating an order to the matcher is identical to `signOrder`, but it also tries to send data to the matcher.
...
Gic Wallet Pro's method for signing cancellation of an order to the matcher. As input, it accepts an object similar to a transaction like this:
...
Gic Wallet Pro's method for cancelling an order to the matcher. It works identically to `signCancelOrder`, but also tries to send data to the matcher. You should specify two more fields: `priceAsset` and `amountAsset` from the order.

Example:

```js
GicWalletPro.signAndPublishCancelOrder({
  type: 1003,
  priceAsset: 'WAVES',
...
Gic Wallet Pro's method for signing typified data, for signing requests on various services. As input, it accepts an object similar to a transaction like this:
...
Gic Wallet Pro's method for signing a custom data for different services. It accepts an object:
...
Check whether the user allowed your website to access Gic Wallet Pro.

Example:

```js
GicWalletPro.resourceIsApproved().then(result => {
  console.log(result);
});
```
...
Check whether the user blocked your website in Gic Wallet Pro.

Example:

```js
GicWalletPro.resourceIsBlocked().then(result => {
  console.log(result);
});
```
